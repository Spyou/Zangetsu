...
import '../../features/companion/companion_settings_screen.dart';
...
class OpenLinkService {
  ...
  void _openRemote(Uri uri, [int attempt = 0]) {
    final nav = rootNavigatorKey.currentState;
    if (nav == null) {
      if (attempt < 30)
        Future.delayed(
          const Duration(milliseconds: 250),
          () => _openRemote(uri, attempt + 1),
        );
      return;
    }
    nav.push(
      MaterialPageRoute<void>(builder: (_) => const CompanionSettingsScreen()),
    );
    unawaited(
      RemoteSession.instance.connectQr(uri.toString()).catchError((_) {}),
    );
  }
